...
import { ONBOARDING_GUIDE_STEPS, ONBOARDING_GUIDE_TIPS } from "@/constants/sellerOnboarding.js";

export function OnboardingGuideSidebar({ open, onClose }) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-label="Seller onboarding guide"
        className={cn(
      ...
      >
        <div className="flex items-center justify-between border-b border-brand-amber/15 bg-gradient-to-r from-brand-cream/50 to-brand-white px-5 py-4">
          <div>
            ...
            <h2 className="mt-1 text-lg font-black tracking-tight">Onboarding guide</h2>
          </div>
          <Button
            type="button"
            variant="ghost"
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-5">
          <p className="text-sm leading-relaxed text-muted-foreground">
            This guide explains each section of the seller application so you can prepare
            everything before you submit.
          </p>

          <div className="space-y-4">
            {ONBOARDING_GUIDE_STEPS.map(({ icon: Icon, title, summary, details }, i) => (
              <Card
                key={title}
